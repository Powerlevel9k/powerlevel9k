...
|`truncate_to_unique`|Parse all parent path components and truncate them to the shortest unique length. If you copy & paste the result to a shell, after hitting `TAB` it should expand to the original path unambiguously. You can leave the the last directories untouched by using different values for `P9K_DIR_SHORTEN_LENGTH`. Known issue: `TAB` does not work well for paths containing characters that have to be escaped, e.g. " ".|
