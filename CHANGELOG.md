...
- `utilities.zsh`
  ...
  - Added function `__p9k_update_environment_vars()` to automatically change user variables for backward compatibility. NOTE: There may be minor issues with arrays, so this still needs review.
- `icons.zsh`
...
- Moved all segments to their own files under `segments`.
  - Segments now use `p9k::prepare_segment()` instead of `$1_prompt_segment()`.
...
- Internal changes:
  ...
  - Replaced `$1_prompt_segment()` with `p9k::prepare_segment()` to enable single call from segments irrespective of generator.
  - Changed all internal functions to use `camelCase`. Exceptions are listed below:
    ...
    - `prompt_powerlevel9k_setup()`, `prompt_powerlevel9k_teardown()`, and `__p9k_preexec()`.
  - `default.p9k`
