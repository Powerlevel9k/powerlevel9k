...
left_prompt_segment() {
  ...
  if [[ $CURRENT_BG != 'initial' ]] && ! isSameColor "${backgroundColor}" "$CURRENT_BG"; then
...
right_prompt_segment() {
  ...
  # If CURRENT_RIGHT_BG is "initial", we are the first right segment.

  if [[ "$CURRENT_RIGHT_BG" != "initial" ]]; then
    # This is the closing whitespace for the previous segment
    echo -n "${POWERLEVEL9K_WHITESPACE_BETWEEN_RIGHT_SEGMENTS}%f"
  fi

  if [[ $joined == false ]] || [[ "$CURRENT_RIGHT_BG" == "initial" ]]; then
